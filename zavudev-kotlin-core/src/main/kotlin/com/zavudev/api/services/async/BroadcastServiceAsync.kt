...
interface BroadcastServiceAsync {
     ...
     *
     * **Sending a broadcast needs no account verification.** Identity and business verification
     * raise daily ceilings; neither is a permission to broadcast, on any channel. What stands in
     * front of a broadcast is the content review below, and it applies to every send path —
     * dashboard, API and CLI. Drafts can be created, edited and kept freely.
     *
     ...
     * email broadcast sends as soon as the review passes it, unless the review asks for a person.
     * Every other channel moves to `pending_admin_review` and waits for a person. A broadcast the
     * review refuses lands on `rejected`: use PATCH to edit the content then call POST
     * /retry-review, or escalate it for a manual review.
     *
     * A broadcast is read once, on its own text, rather than per recipient. While an account's
     * sending is suspended its recipients fail individually with `errorCode` `SENDING_SUSPENDED`;
     * see `POST /v1/messages`.
     *
