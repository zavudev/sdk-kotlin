...
interface MessageServiceAsync {
     ...
     *   deposit, or subscribe to a paid plan. Business verification (KYB) is required for **one
     *   channel only**: `sms_oneway`. Without an approved KYB, one-way SMS returns `403` with code
     *   `kyb_required` and `details.dashboardUrl` pointing at `/kyb`, whatever the account has
     *   otherwise verified. No other channel asks for it
