 ...
 *   Free. **It counts messages in both directions**: a message a contact sends you consumes one
 *   unit exactly as a message you send them does, so a project that has sent 300 and received 1,700
 *   has used the whole allowance. Messages you send from the WhatsApp Business App on your own
 *   phone under coexistence are mirrored into your inbox but never counted, and neither are failed
 *   sends. Over the allowance, sends return 429 with code `a2p_limit_exceeded` and upgrade details,
 *   **and inbound messages on those channels are refused as well**: not stored, not shown in the
 *   inbox, and no `message.inbound` webhook, and not delivered later when the month resets. The
 ...
 *
 * **Daily limits:**
 * - An account sends on every channel from its first minute, to any destination. Verification is
 *   not a permission to send: identity verification and business verification (KYB) raise the
 *   ceilings below and nothing else asks for them here. KYB is still required to register a 10DLC
 *   brand and campaign, which every US and Canadian (+1) SMS destination needs — a carrier rule,
 *   answered separately with `403 ten_dlc_required`
 ...
 * - Email: a sender with a verified domain sends from day one, within the plan quota (100/day and
 *   3,000/month on Free). Over the daily quota it returns `429` with code `daily_limit_exceeded`
 * - Full reference: https://docs.zavu.dev/concepts/sending-limits
 *
 * **Risk review:** Every outbound `sms`, `sms_oneway`, `email` and `voice` message is read before
 * it is sent — the content, and how this account has been sending. What is checked is the message,
 * not who you are.
 * - A message can be **held** for a short review. It stays `queued` while it waits: no new status
 *   exists for this, and `MessageStatus` is unchanged. When it is approved it sends normally.
 * - A message that is not approved moves to `failed` and fires `message.failed`. `errorCode` says
 *   which happened: `RISK_REJECTED` (a reviewer refused it), `RISK_REVIEW_EXPIRED` (the review
 *   window closed first — it is a couple of hours, because a code that arrives late is worse than
 *   one that does not arrive), or `RISK_BLOCKED` (refused outright, without a hold). An SMS that
 ...
 *   hours late.
 * - A message whose content cannot be read — the check is briefly unavailable — is held rather than
 *   sent. An account with an approved business verification is unaffected, and so is one that has
