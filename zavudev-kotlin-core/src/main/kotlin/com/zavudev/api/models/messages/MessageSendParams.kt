 ...
 *   paid plan. Business verification (KYB) is required for **one channel only**: `sms_oneway`.
 *   Without an approved KYB, one-way SMS returns `403` with code `kyb_required` and
 *   `details.dashboardUrl` pointing at `/kyb`, whatever the account has otherwise verified. No
 *   other channel asks for it
 ...
 *   past the unverified level, see `POST /v1/broadcasts/{broadcastId}/send`
