 ...
 *   dispatched the webhook) to detect and ignore delayed deliveries. When the conversation was
 *   opened from a Click-to-WhatsApp ad or post, `data.referral` carries the ad attribution —
 *   including `ctwaClid`, the identifier Meta's Conversions API needs to credit a conversion back
 *   to that ad. WhatsApp only, and only on the first message of the thread: it is absent from every
 *   later message, so persist it when it arrives.
 ...
 * - `template.status_changed`: WhatsApp template approval status changed. `data` carries
 *   `templateId`, `name`, `previousStatus`, `currentStatus`, `rejectionReason`, and `category` —
 *   the category Meta currently bills the template under. Meta can recategorize a template
 *   (typically `UTILITY` to `MARKETING`) at approval or long afterwards, which changes what each
 *   message costs; `category` is how that reaches you. A recategorization with no status change is
 *   delivered as this same event, so compare `category` against what you hold rather than only
 *   reacting to `currentStatus`.
 *
 * **Partner events:**
