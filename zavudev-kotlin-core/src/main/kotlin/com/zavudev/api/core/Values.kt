...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw ZavudevInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ZavudevInvalidDataException("`$name` is not set")
            is JsonNull -> throw ZavudevInvalidDataException("`$name` is null")
            else -> throw ZavudevInvalidDataException("`$name` is invalid, received $this")
        }
